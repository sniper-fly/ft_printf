...
/*   Updated: 2020/08/17 13:20:42 by rnakai           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				output_x(t_flags *flags, va_list argptr)
{
	...
	output_x2(flags, num, actual_width - actual_numlen, base);
}
						...
						int actual_width_minus_numlen, char *base)
{
	...
	else if (flags->fill_zero_flag)
	{
		...
		{
			put_it_xx_times('0', actual_width_minus_numlen);
...
void				output_largex(t_flags *flags, va_list argptr)
{
	...
	actual_width = MAX(actual_numlen, flags->width);
	flags->total_output_len += actual_width;
	output_largex2(flags, num, actual_width - actual_numlen, base);
}
